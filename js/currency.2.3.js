...
function getYearMonthInDescOrder(numMonths) {
  ...
  for (let i = 0; i < numMonths; i++) {
	...
	if(i == 0) {
		...
		isURLReachable(url)
		  .then((result) => {
			console.log(result);
			result += `<csv>currency_${month}.1.${year}.csv</csv>`;
		  })
		  .catch((error) => {
...
async function isURLReachable(url) {
  // use for checking if url exists in function getYearMonthInDescOrder 
  try {
    const response = await fetch(url);
    if (response.ok) {
      return true; // URL can be reached
    } else {
